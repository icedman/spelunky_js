...
double REAL_UNIT_INT = 1.0 / ((double)MAX_INT + 1.0);
double REAL_UNIT_UINT = 1.0 / ((double)MAX_UINT + 1.0);
...
int NextUpper(int upperBound) {
  ...
  // performance. See comments in NextDouble.
...
int NextLowerUpper(int lowerBound, int upperBound) {
  ...
  return lowerBound + (int)((REAL_UNIT_INT *
                             (double)(int)(0x7FFFFFFF & (w = (w ^ (w >> 19)) ^
                                                             (t ^ (t >> 8))))) *
                            (double)range);
}
...
double NextDouble() {
  ...
  // explicitly cast to an int the compiler will then cast the int to a double
  // to perform the multiplication, this final cast is a lot faster than casting
  // from a uint to a double. The extra cast to an int is very fast (the
